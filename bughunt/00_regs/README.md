...
Как мы помним, при подключении тестируемого ядра в топ-модуль верификационного окружения мы дали имя экземпляру процессора `DUT`. Теперь в GTKWave мы иерархически обращаемся к этому модулю для получения информации об изменении его сигналов.
...
На временной диаграмме не наблюдаем записи значения 1 в регистр 3 (1), а в память по адресу 64 (в 16 ячейку) вместо значения 1 записывается значение 5 (2). Ядро некорректно выполняет программу.
...
Давайте обратим внимание на сигналы `cu_force_f_i` и `cu_force_pc_i`. Есть подозрение, что при операциях ветвления именно они отвечают за формирование нового счетчика команд, так как в иных случаях в `next_pc` попадает либо текущий счетчик команд `fetched_pc_ff` (случай приостановки конвейера), либо адрес следующей инструкции `pc_plus_inc`. Отправим сигналы `cu_force_f_i` и `cu_force_pc_i` на временную диаграмму.
...
Поразмыслив над логикой формирования, можно сделать вывод, что данный сигнал равен 1 тогда, когда предсказание условного перехода (ветвления)[^2], сделанное на этапе декодирования (decode stage), не совпадает с реальным результатом ветвления, вычисленным на этапе выполнения (execute stage) (внимание на [микроархитектурную схему ядра](#тестируемое-risc-v-ядро)).
...
**Обратим особое внимание** на сигнал `branch_des`. Он формируется арифметико-логическим устройством (АЛУ). Подключение АЛУ представлено в модуле [`miriscv_execute_stage.sv`](https://github.com/riscv-tests-intro/MIRISCV/tree/bug_00/miriscv/rtl/miriscv_execute_stage.sv):
